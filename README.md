...
AI 检查（持久化任务队列：抢占执行 → 超时重试 3 次 → 耗尽标记最终失败）
		...
		└─ 通过 → ai_reviewed（送审不可关闭的强制前置）
专家多轮审核与决断
		├─ 需修改 → 题目恢复 ai_reviewed，退回生成者修改（「待我修改」页），保存新版本后按原流程从第一轮重新送审
...
- AI 检查仅在首次生成后自动执行一次；系统故障按同一任务自动重试，耗尽后阻断流程并保留错误，不提供人工复检或强制通过。`review:do`（审题）默认只见“AI 预审”折叠标识；`review:final` 在决断工作台可见首次检查报告与全部轮次专家评语。
