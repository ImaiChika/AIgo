...
| **AI 检查** | 生成/导入/编辑后自动触发，LLM 检查题目质量（科学性、答案、解析、A2格式），四维评分；命题元数据只作提示词反馈，不因标签不一致淘汰题目；送审强制前置 |
...
- 单题出题与批量推理使用独立的 `question:generate` / `batch:run` 权限；默认只有超级管理员继承批量推理，管理员、命题教师和审题专家需由超级管理员在用户权限矩阵中单独授予。未分配批量权限时隐藏批量入口，直接访问批量地址会返回单题页并提示权限不足。
