...
test("all existing destinations belong to five distinct groups; batch shares the generation entry", () => {
  ...
  const paths = groups.flatMap(g => g.items.map(i => i.path));
	...
	assert.deepEqual(new Set(paths), new Set(["/knowledge", "/generate", "/my-revisions", "/share-requests", "/review", "/review-decisions", "/review-results", "/review-flows", "/bank", "/banks", "/stats", "/users", "/roles", "/system/ai-providers", "/audit"]));
...
test("permission filtering removes inaccessible children and empty categories", () => {
  ...
  assert.deepEqual(reviewer[1].items.map(i => i.id), ["review"]);
  const finalReviewer = visibleNavigation(permissions("review:final"));
  assert.deepEqual(finalReviewer[1].items.map(i => i.id), ["review-decisions"]);
